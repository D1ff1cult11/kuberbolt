...
func main() {
	...
	if err == nil {
		...
		data = config.TestData{
			...
			GuardrailParams: config.GuardrailParams{
				MaxPaymentSats: 1000,
			},
		}
	}

	if data.GuardrailParams.MaxPaymentSats <= 0 {
		data.GuardrailParams.MaxPaymentSats = 1000
	}
	...
	fmt.Printf("[+] Payment Guardrail Active: Max payment allowed = %d sats\n", data.GuardrailParams.MaxPaymentSats)
